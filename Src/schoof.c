     ...
     il existe un point P non trivial de E[l] avec (Phi_l)^2*P = ±q_bar*P
        ...
        */
        ...
        if (!fq_poly_is_one(poly, ctx)) {
            ...
            fmpz_one(j);
            fmpz_sub_ui(tmp, l, 1);
            fmpz_divexact_ui(tmp, tmp, 2);// (l-1)/2
            while (fmpz_cmp(j, tmp) <= 0) {
                ...
                fq_poly_print_pretty(ordo, "x", ctx) ;flint_printf("\n");
                fq_poly_print_pretty(absc, "x", ctx) ;flint_printf("\n\n");
                if (!fq_poly_is_one(poly, ctx)) {
...
int main() {
    ...
    elliptic_curve E;
    fq_set_si(E.a, 100, ctx);
    fq_set_si(E.b, 0, ctx);

    /*
    fmpz_t k;fmpz_init(k);fmpz_set_si(k,10);
